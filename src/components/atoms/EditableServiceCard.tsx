...
const EditableServiceCard: React.FC<EditableServiceCardProps> = ({
  ...
  return (
    <>
      <div
        onClick={handleClick}
        className={`group p-8 bg-white border-2 border-gray-200 hover:border-gray-900 transition-all duration-200 ${editClasses} ${selectedClasses}`}
      >
        ...
        <ul className="space-y-2">
          {service.features.map((feature, idx) => (
            <li key={idx} className="flex items-start text-sm text-gray-600">
              <span className="font-mono mr-3 text-gray-400">→</span>
              {feature}
            </li>
          ))}
        </ul>
